...
If running embient from a virtual environment:
```bash
...
Replace `[YOUR_SKILLS_DIR]` with the absolute skills directory path from your system prompt (e.g., `~/.embient/agent/skills` or the full absolute path).
...
```bash
.venv/bin/python ~/.embient/agent/skills/arxiv-search/arxiv_search.py "deep learning drug discovery" --max-papers 5
```

Search for computational biology papers:
```bash
.venv/bin/python ~/.embient/agent/skills/arxiv-search/arxiv_search.py "protein folding prediction"
```

Search for bioinformatics methods:
```bash
.venv/bin/python ~/.embient/agent/skills/arxiv-search/arxiv_search.py "genome assembly algorithms"
```
...
If running embient from a virtual environment (recommended), use the venv's Python:
```bash
...
The package is not included in embient by default since it's skill-specific. Install it on-demand when first using this skill.
