...
- embient skills list --agent <agent> [--project]
- embient skills create <name>
- embient skills info <name>
"""
...
from embient.config import COLORS, Settings, console
...
def _list(agent: str, *, project: bool = False) -> None:
    ...
    if project:
        ...
        if not project_skills_dir.exists() or not any(project_skills_dir.iterdir()):
            ...
            console.print(
                "\n[dim]Create a project skill:\n  embient skills create my-skill --project[/dim]",
                style=COLORS["dim"],
            )
            return
        ...
        if not skills:
            ...
            console.print(
                "[dim]Skills will be created in ~/.embient/agent/skills/ when you add them.[/dim]",
                style=COLORS["dim"],
            )
            console.print(
                "\n[dim]Create your first skill:\n  embient skills create my-skill[/dim]",
                style=COLORS["dim"],
            )
            return
...
"""
    ...
    console.print(
        ...
        f"  nano {skill_md}\n"
        "\n"
        "💡 See examples/skills/ in the embient repo for example skills:\n"
        "   - web-research: Structured research workflow\n"
        "   - langgraph-docs: LangGraph documentation lookup\n"
        "\n"
        "   Copy an example: cp -r examples/skills/web-research ~/.embient/agent/skills/\n",
        style=COLORS["dim"],
    )
...
def execute_skills_command(args: argparse.Namespace) -> None:
    ...
    if args.skills_command == "list":
        ...
        console.print("  embient skills <command> [options]\n")
        ...
        console.print("  embient skills list")
        console.print("  embient skills create web-research")
        console.print("  embient skills info web-research")
        console.print("\n[dim]For more help on a specific command:[/dim]", style=COLORS["dim"])
        console.print("  embient skills <command> --help", style=COLORS["dim"])
