...
def show_help() -> None:
    ...
    console.print(
        "  embient --model gpt-4o               # Use specific model (auto-detects provider)",
        style=COLORS["dim"],
    )
    console.print(
        "  embient -r                           # Resume most recent session",
        style=COLORS["dim"],
    )
    console.print(
        "  embient -r abc123                    # Resume specific thread",
        style=COLORS["dim"],
    )
    console.print(
        "  embient --auto-approve               # Start with auto-approve enabled",
        style=COLORS["dim"],
    )
    console.print(
        "  embient --sandbox runloop            # Execute code in Runloop sandbox",
        style=COLORS["dim"],
    )
    console.print()

    console.print("[bold]Authentication:[/bold]", style=COLORS["primary"])
    console.print("  embient login                        # Authenticate with Embient")
    console.print("  embient logout                       # Clear stored credentials")
    console.print("  embient status                       # Show authentication status")
    console.print()
